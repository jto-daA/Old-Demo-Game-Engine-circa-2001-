...
static long IG_3DS_FindChunkID( k_word chunkID , FILE* p3DS_BinFile )
...
// yes.... this qualifies as a hideous hack....
{
	...
	long     objOffset = IG_3DS_FindChunkID( IG_3DS_KEYFRAME_NODE_OBJ , p3DS_BinFile ),
		 ...
		 nodeHdrOffset = IG_3DS_FindChunkID( IG_3DS_KEYFRAME_NODE_OBJ_NODE_HDR , p3DS_BinFile ), 
		  ...
		  unkwnObjectRef,unkwnNodeRef,unkwnPivotRef,unkwnPosKeyRef1,unkwnPosKeyRef2,unkwnRotKeyRef1;
	...
	for( index = 0; index < keyframeData->globalKeyframe.animationLenght-1;index++)
	//  be aware that the conditon "keyframeData->pRotationTracks[0].rotTrackHeader.trackKeyCount" may have to be evaluated...
	{
		keyframeData->pRotationTracks[ index ].rotKeyHeader.frameIndex = UTL_SwitchEndian_ReadDWordFromFile( p3DS_BinFile , ftell(p3DS_BinFile) );
		tempFlag = UTL_SwitchEndian_ReadWordFromFile( p3DS_BinFile , ftell(p3DS_BinFile) );
		
		if( tempFlag & 0x0001 )
		{
			fread( &keyframeData->pRotationTracks[ index ].rotKeyHeader.splineTension , sizeof(float) , 1 , p3DS_BinFile );
		}
		
		if( tempFlag & 0x0002 )
		{
			fread( &keyframeData->pRotationTracks[ index ].rotKeyHeader.splineContinuity , sizeof(float) , 1 , p3DS_BinFile );
		}

		if( tempFlag & 0x0004 )
		{
			fread( &keyframeData->pRotationTracks[ index ].rotKeyHeader.splineBias , sizeof(float) , 1 , p3DS_BinFile );
		}

		if( tempFlag & 0x0008 )
		{
			fread( &keyframeData->pRotationTracks[ index ].rotKeyHeader.keyEaseToValue , sizeof(float) , 1 , p3DS_BinFile );
		}

		if( tempFlag & 0x0010 )
		{
			fread( &keyframeData->pRotationTracks[ index ].rotKeyHeader.keyEaseFromValue , sizeof(float) , 1 , p3DS_BinFile);
		}

		// convert these values from degrees to radians...
		fread( &keyframeData->pRotationTracks[ index ].rotationAngle , sizeof(float) , 1 , p3DS_BinFile );
		keyframeData->pRotationTracks[ index ].rotationAngle *= 180 / PI;
		
		fread( &keyframeData->pRotationTracks[ index ].trackRotationVector.xComponent , sizeof(float) , 1 , p3DS_BinFile );
		//keyframeData->pRotationTracks[ index ].trackRotationVector.xComponent *= 180 / PI;
		
		fread( &keyframeData->pRotationTracks[ index ].trackRotationVector.zComponent , sizeof(float) , 1 , p3DS_BinFile );
		//keyframeData->pRotationTracks[ index ].trackRotationVector.xComponent *= 180 / PI;

		fread( &keyframeData->pRotationTracks[ index ].trackRotationVector.yComponent , sizeof(float) , 1 , p3DS_BinFile );
		//keyframeData->pRotationTracks[ index ].trackRotationVector.xComponent *= 180 / PI;
